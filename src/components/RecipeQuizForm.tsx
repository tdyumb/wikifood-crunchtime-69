...
const timeToMakeOptions = [
  { id: 'time-short', label: 'Quick (20 mins or less)' },
  { id: 'time-medium', label: 'Moderate (20-30 mins)' },
  { id: 'time-long', label: 'Longer (30+ mins)' },
];
...
const RecipeQuizForm: React.FC<RecipeQuizFormProps> = ({ onSubmit }) => {
  ...
  const handleFindRecipes = () => {
    ...
    const newFilterState = {
      ...filters,
      mealType: mealType ? [mealType.toLowerCase()] : filters.mealType,
      dietaryRestrictions: selectedDietaryNeeds.length > 0 ? selectedDietaryNeeds : filters.dietaryRestrictions,
      cuisineType: [] // Clear cuisine types as requested
    };
      ...
      case 1:
        return (
          <motion.div key="step1" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-4">
            <h3 className="text-xl font-semibold">Time Available</h3>
      ...
      case 2:
        return (
          <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-4">
            ...
            <p className="text-sm text-gray-600">What meal are you planning to make?</p>
            <RadioGroup defaultValue={answers.mealType} onValueChange={(value) => handleRadioChange('mealType', value)} className="grid grid-cols-2 gap-4">
